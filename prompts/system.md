...
✓ **CHECK 5 — Length Comment Present**: Every question MUST end with `<!-- Lengths: C=XX | D1=XX | D2=XX | D3=XX -->`. Missing = VIOLATION.

🚨 CRITICAL ENFORCEMENT:
- If ANY question fails ANY check, YOU MUST REWRITE THAT QUESTION'S ANSWERS — you are FORBIDDEN from emitting output with violations
- DO NOT emit output "hoping it passes" — verify FIRST, emit ONLY AFTER all checks pass
- The length comment is MANDATORY proof that you performed verification — no comment = no verification performed = REJECTED output
